 ...
 * API: Read
 * =====================
 * Read username, get url of post, get description, etc...
 *
...
class Read {
     ...
     * Post ID from URL
     * =====================
     * Get hash of ig post url
     *
     * @param {string}  post_url  - ig post url (mandatory)
     *
     * @return {Object}  response          - {}
     *         {boolean} response.status   - true: successful / false: fail
     *         {string}  response.post_id - string of hash id, empty if get error
     *         {string}  response.error    - if status is false return error details
     *
     * @since: v0.10
     *
     */
	async post_hash_id(post_url) {
		let tag = "read::post_hash_id()";
		this.log.info(tag, `${this.lang.translate("try_get_post_hash_id")}`);

		let response = {"status": false, post_id: ""};

		try {
			response.post_id = post_url.split("/")[4];

			this.log.info(tag, `${response.post_id}`);
		} catch (err) {
			response.post_id = "";
			response.status = false;
			response.error = err;
     ...
     * Posts
     * =====================
     * Read array of posts url
     *
     * @todo: auto mode for <where> param: detect correct page automatically
     *
     * @param {string}  where  - get urls of user posts from profile page or hashtag page, available values: profile, hashtag, location (optional: default profile)
     *
     * @return {Object}  response             - {}
     *         {boolean} response.status      - true: successful / false: fail
     *         {Array}   response.posts_urls - post urls, empty array if get error
     *         {Array}   response.posts_ids  - post hash ids, empty array if get error
     *         {string}  response.error       - if status is false return error details
     *
     * @since: v0.10
     *
     */
	async posts_list(where = "profile") {
		let tag ="read::posts_list()";
		this.log.info(tag, `${this.lang.translate("try_get_post_array")}`);

		let response = {"status": false, posts_urls: [], posts_ids: []};
		let selector = "";
		...
		try {
			await this.core.bot.waitForSelector(selector, {timeout: 5000});
			response.posts_urls = await this.core.bot.$$eval(selector, hrefs => hrefs.map((a) => {
				return a.href;
			}));

			for (let i = 0; i < response.posts_urls.length; ++i) {
			    response.posts_ids.push(response.posts_urls[i].split("/")[4]);
			}

			this.log.debug(tag, `${this.lang.translate("post_urls")}: ${response.posts_urls}`);
			this.log.debug(tag, `${this.lang.translate("post_ids")}: ${response.posts_ids}`);
			response.status = true;
		} catch (err) {
			response.posts_urls = [];
			response.posts_ids = [];
