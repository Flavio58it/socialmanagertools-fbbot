/**
 * MODE: likemode_friendsfeed_realistic
 * =====================
 * Goto random hashtag from config list, like fast 12 post and sleep X min in night.
 *
...
class Likemode_friendsfeed_realistic extends Status {
	constructor() {
		super();
		this.core = core;
		...
		this.local.cache_posts_ids = [];
		this.local.current_post_id = "";
		...
		do {
			...
			if (!is_sleep_night_flag_enabled || is_day) {
				...
				if (this.local.cache_hashtags.length <= 0) {
					this.local.cache_hashtags = this.core.config.bot_mode_options[this.core.config.bot_mode].facebook_hashtag.slice();
				}

				if (this.local.like_counter < 1 || this.local.cache_posts_ids <= 0) {
					...
					let response_read_posts_list = await this.api.read.posts_list("hashtag");
					this.local.cache_posts_ids = response_read_posts_list.posts_ids;
				}

				this.log.debug(tag, `${this.lang.translate("hashtag_id")}: #${this.local.current_hashtag}`);

				this.local.current_post_id = this.local.cache_posts_ids.pop();
				await this.api.goto.post(this.local.current_post_id);
				...
				let json = {
					...
					"post_id": this.local.current_post_id,
...
module.exports = Likemode_friendsfeed_realistic;